...
import { LocalFirebaseAPI } from 'app/model/local-firebase-api.service';
...
@Injectable()
export class LocalFirebaseDataSource implements DataSource {
  ...
  constructor(private lafbapi: LocalFirebaseAPI) { }
  ...
  private async fetchU(): Promise<void> {
    let uobjs = await this.lafbapi.objectFirstMap(USERS_URL)
    uobjs.forEach(u => this.holders.users.push(createU(u)))
  }

  private async fetchT(): Promise<void> {
    let tobjs = await this.lafbapi.listFirstMap(TAGS_URL)
    tobjs.forEach(t => {
      let tag = createT(t)
  ...
  private async fetchE(): Promise<void> {
    let eobjs = await this.lafbapi.listFirstMap(EXAMS_URL)
    eobjs.forEach(e => {
  ...
  private async fetchR(user: User): Promise<void> {
    let robjs = await this.lafbapi.listFirstMapR(this.resultsUrl(user))
  ...
  private async fetchC(user: User): Promise<void> {
    let cobjs = await this.lafbapi.listFirstMapR(this.chartsUrl(user))
  ...
  public deleteChart(user: User, cid: string): Promise<boolean> {
    ...
    return this.lafbapi.objectRemoveBool(url)
  }
  ...
  public updateChart(user: User, chart: Chart): Promise<boolean> {
    ...
    return this.lafbapi.objectSetBool(url, co)
  }
  ...
  public createChart(user: User): Promise<Chart> {
    ...
    return this.lafbapi.listPush<Chart>(url, co, call => {
      let key = call.key
  ...
  public updateExam(user: User, result: ExamResult): Promise<boolean> {
    ...
    return this.lafbapi.objectSetBool(url, ro)
  }
  ...
  public createExam(user: User, eid: string): Promise<ExamResult> {
    ...
    return this.lafbapi.listPush<ExamResult>(url, ro, call => {
      let key = call.key
  ...
  public editExamDetail(user: User, type: ExamEditType, diff: any,
    ...
    return this.lafbapi.objectSetBool(url, diff)
  }
  ...
  public defineExam(user: User, exam: Exam): Promise<boolean> {
    ...
    let url = EXAMS_URL
    return this.lafbapi.objectUpdate<boolean>(url, eocover, call => {
  ...
  public addQuestion(user: User, eid: string, question: Question): Promise<boolean> {
    ...
    return this.lafbapi.objectUpdateBool(url, qo)
  }
  ...
  public addLinkQuestion(user: User, eid: string, qid: string, leid: string, lqid: string): Promise<boolean> {
    ...
    return this.lafbapi.objectUpdateBool(url, linkq)
  }

  public publishExam(user: User, eid: string): Promise<boolean> {
    let url = EXAMS_URL + eid + '/status/'
    return this.lafbapi.objectSetBool(url, 'DONE')
  }
  ...
  public saveFile(user: User, eid: string, qid: string, fileLink: FileLink): Promise<string> {
    ...
    return this.lafbapi.listPush<string>(url, fileLink, call => {
      let key = call.key + ''
  ...
  public addGroup(user: User, eid: string, qgroup: QuestionGroup): Promise<boolean> {
    ...
    return this.lafbapi.objectUpdateBool(url, go)
  }
  ...
  public createTag(user: User, title: string): Promise<Tag> {
    ...
    to['title'] = title
    let url = TAGS_URL
    return this.lafbapi.listPush<Tag>(url, to, call => {
      let key = call.key
